...
@section('content')
    <header class="h-16 bg-white border-b border-slate-200 flex justify-between items-center px-8 z-10">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('web.home') }}" class="text-sm font-semibold text-slate-500 hover:text-sonec-green flex items-center gap-2">
        ...
        <div id="view-blog" class="max-w-5xl mx-auto">
            <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                ...
                <form id="create-article-form" class="space-y-4 overflow-y-auto custom-scrollbar" enctype="multipart/form-data">
                    ...
                    <div>
                        {{-- voir  l'image de couverture enregistrée --}}
                        <div>
                            <img id="current-image" src="{{ $article->image_url ?? asset('storage/' . $article->image_article) }}" alt="{{ $article->title }}" class="w-32 h-32 object-cover rounded-md mb-4">
                        ...
                        <div class="mt-6">
                            ...
                            <input type="text" id="image_url" name="image_url" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-normal text-sm" placeholder="https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-5.jpg" value="{{ $article->image_url }}">
                        </div>
                    ...
                    <div class="mt-6">
                        ...
                        <input type="text" id="title" name="title" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-md" placeholder="INNOVATION ECOLEWEB : L'IA AU SERVICE D'UN APPRENTISSAGE PERSONNALISÉ" value="{{ $article->title }}">
                        <input type="hidden" readonly id="slug" name="slug" class="mt-4 w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-md" value="{{ $article->slug }}">
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-4">
                        <div>
                            ...
                            <input type="date" id="published_at" name="published_at" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm" placeholder="2024-01-15" value="{{ $article->published_at->format('Y-m-d') }}">
                        </div>
                        <div>
                            ...
                            <select name="category_id" id="category_id" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm">
                                @foreach($categorie_articles as $categorie)
                                    <option value="{{ $categorie->id }}" {{ $article->category_id == $categorie->id ? 'selected' : '' }}>{{ $categorie->label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            <label for="temps_lecture" class="block text-xs font-bold text-slate-400 uppercase mb-1">Temps de lecture</label>
                            <input type="text" id="temps_lecture" name="temps_lecture" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm" placeholder="8 min" value="{{ $article->temps_lecture }}">
                        </div>
                    </div>
                    <div class="mt-6">
                        ...
                        <textarea id="description_courte" name="description_courte" class="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono" rows="6" >{{ $article->description_courte }}</textarea>
                    </div>
                    <div class="mt-6">
                        <label for="notes" class="block text-xs font-bold text-slate-400 uppercase mb-1">Ajouter une note</label>
                        <textarea id="notes" name="notes" class="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono" rows="4">{{ $article->notes }}</textarea>
                    </div>
                    <div class="mt-6">
                        ...
                        <textarea id="content" name="content" class="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono" rows="20">{{ $article->content }}</textarea>
                    </div>
                        ...
                        <div class="container mx-auto px-6 lg:px-12">
                            <div class="max-w-4xl mx-auto">
                                ...
                                <div class="flex flex-wrap gap-3">
                                    @foreach ($tags as $tag )
                                        <label class="px-5 py-2.5 {{ in_array($tag->id, $article->tags->pluck('id')->toArray()) ? 'bg-sonec-green text-white' : 'bg-white text-sonec-dark' }} border border-gray-200 rounded-full text-sm font-medium hover:border-sonec-green hover:text-sonec-green hover:shadow-sm transition-all cursor-pointer">
                                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="hidden peer" {{ in_array($tag->id, $article->tags->pluck('id')->toArray()) ? 'checked' : '' }}>
                                            {{ $tag->name }}
                                        </label>
                                    @endforeach
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-4">
                        <div>
                            ...
                            <input id="author" name="author" type="text" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm" placeholder="Equipe Sonec Africa" value="{{ $article->author }}">
                        </div>
                        <div>
                            <label for="position_auteur" class="block text-xs font-bold text-slate-400 uppercase mb-1">Position/Fonction</label>
                            <input type="text" id="position_auteur" name="position_auteur" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm" placeholder="Direction Innovation" value="{{ $article->position_auteur }}">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="photo_auteur_url" id="photo_auteur_url" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm" placeholder="https://storage.googleapis.com/uxpilot-auth.appspot.com/avatars/avatar-5.jpg" value="{{ $article->photo_auteur_url }}">
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" id="a_la_une" name="a_la_une" {{ $article->a_la_une ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-sonec-green focus:ring-sonec-green">
                            <label for="a_la_une" class="block text-sm font-medium text-slate-700">Article à la une</label> 
                        </div>
                        <div class="flex items-center gap-2">
                            <input type="checkbox" id="activer_partage_reseaux_sociaux" name="activer_partage_reseaux_sociaux" {{ $article->activer_partage_reseaux_sociaux ? 'checked' : '' }}  class="form-checkbox h-5 w-5 text-sonec-green focus:ring-sonec-green">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="checkbox" id="is_published" name="is_published" {{ $article->is_published ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-sonec-green focus:ring-sonec-green">
    ...
    <script>
        ...
        function saveAll(e){
            ...
            fetch("{{ route('articles.update',['article' => $article->id]) }}", {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                if (data.success===true) {
                    ...
                    successDescription.textContent = data.description || 'Article modifié avec succès.';
                    ...
                    setTimeout(() => {
                        ...
                        window.location.href = "{{ route('admin.actualites') }}";
                    }, 1200);                    
