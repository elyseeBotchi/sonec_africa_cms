<div class="space-y-8">
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        ...
        <input type="hidden" name="seo_id" value="{{ $secteurExpertise->seo->id ?? '' }}">
        <div class="mb-4">
            <div>
                <label for="seo_title" class="block text-xs font-bold text-slate-400 uppercase mb-1">Meta Title</label>
                <input type="text" id="seo_title" name="seo_title" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold" value="{!! $secteurExpertise->seo->meta_title ?? '' !!}">
            </div>
        ...
        <div class="mt-4">
            ...
            <textarea id="seo_keywords" name="seo_keywords" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono text-blue-500" rows="3">{!! $secteurExpertise->seo->meta_keywords ?? '' !!}</textarea>
        </div>
        <div class="mt-4">
            <label for="seo_description" class="block text-xs font-bold text-slate-400 uppercase mb-1">Meta Description</label>
            <textarea id="seo_description" name="seo_description" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm" rows="4" maxlength="160">{!! $secteurExpertise->seo->meta_description ?? '' !!}</textarea>
