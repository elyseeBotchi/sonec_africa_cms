...
<div class="bg-white p-8 mb-8 rounded-3xl shadow-sm border border-slate-100">
    <div class="flex items-center justify-between">
        <h3 class="text-lg font-bold text-sonec-dark mb-6 flex items-center gap-2"><i class="fas fa-cogs text-green-500"></i> Section "Chiffres clés"</h3>
    ...
    <div id="chiffres-container" class="space-y-6">
        <div class="grid md:grid-cols-2 gap-6" id="chiffres-grid">
            @foreach ($chiffres_presentation as $index => $chiffre)
...
<div class="bg-white p-8 mb-8 rounded-3xl shadow-sm border border-slate-100">
    <h3 class="text-lg font-bold text-sonec-dark mb-6 flex items-center gap-2"><i class="fas fa-handshake text-yellow-500"></i> Section "Présentation"</h3>
    <div class="space-y-4">
        
        <input type="hidden" id="section_presentation_section_key" name="section_presentation_section_key" value="presentation">
        <div>
            <label for="section_presentation_title" class="block text-xs font-bold text-slate-400 uppercase mb-1">Titre</label>
            <input type="text" id="section_presentation_title" name="section_presentation_title" value="{{ $section_presentation->title ?? '' }}" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-lg text-sonec-dark focus:ring-2 focus:ring-yellow-500 outline-none">
        </div>
        <div>
            <label for="section_presentation_description" class="block text-xs font-bold text-slate-400 uppercase mb-1">Description</label>
            <textarea id="section_presentation_description" name="section_presentation_description" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-yellow-500 outline-none" rows="4">{{ $section_presentation->description ?? '' }}</textarea>
        </div>
    </div>
    {{-- <div class="mb-8 mt-4">
        <label for="section_presentation_image_url" class="block text-xs font-bold text-slate-400 uppercase mb-1">Url de l'image</label>
        <input type="text" id="section_presentation_image_url" name="section_presentation_image_url" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono text-blue-500" value="{{ $section_presentation->image_url ?? '' }}">   
    </div> --}}
</div>

{{-- Section mot du directeur --}}
<div class="bg-white p-8 mb-8 rounded-3xl shadow-sm border border-slate-100">
    <h3 class="text-lg font-bold text-sonec-dark mb-6 flex items-center gap-2"><i class="fas fa-handshake text-yellow-500"></i> Section "Mot du directeur"</h3>
    <div class="space-y-4">
        
        <input type="hidden" id="section_vision_section_key" name="section_vision_section_key" value="vision">
        <div>
            <label for="section_vision_title" class="block text-xs font-bold text-slate-400 uppercase mb-1">Titre</label>
            <input type="text" id="section_vision_title" name="section_vision_title" value="{{ $section_vision->title ?? '' }}" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-lg text-sonec-dark focus:ring-2 focus:ring-yellow-500 outline-none">
        </div>
        <div>
            <label for="section_vision_description" class="block text-xs font-bold text-slate-400 uppercase mb-1">Description</label>
            <textarea id="section_vision_description" name="section_vision_description" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-yellow-500 outline-none" rows="4">{{ $section_vision->description ?? '' }}</textarea>
        </div>
    </div>
    {{-- <div class="mb-8 mt-4">
        <label for="section_vision_image_url" class="block text-xs font-bold text-slate-400 uppercase mb-1">Url de l'image</label>
        <input type="text" id="section_vision_image_url" name="section_vision_image_url" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono text-blue-500" value="{{ $section_vision->image_url ?? '' }}">   
    </div> --}}
</div>

{{-- section chiffres visions --}}
<div class="bg-white p-8 mb-8 rounded-3xl shadow-sm border border-slate-100">
    <div class="flex items-center justify-between">
        <h3 class="text-lg font-bold text-sonec-dark mb-6 flex items-center gap-2"><i class="fas fa-cogs text-green-500"></i> Section "Chiffres clés vision"</h3>
        {{-- <h3 class="text-lg font-bold text-sonec-dark mb-6">Carousel d'images d'accueil</h3> --}}
        {{-- <h2 class="text-2xl font-bold text-sonec-dark">Gestion du contenu de la page d'accueil</h2> --}}
        <button onclick="addChiffreVision() " class="flex items-center gap-2 bg-sonec-green text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">
            <i class="fas fa-plus"></i> Ajouter un chiffre clé
        </button>
    </div>
    <div id="chiffres-vision-container" class="space-y-6">
        <div class="grid md:grid-cols-2 gap-6" id="chiffres-vision-grid">
            @foreach ($chiffres_vision as $index => $chiffre)
                <div data-chiffre-vision class="bg-white border border-slate-200 rounded-xl p-4 relative" data-id="{{ $chiffre->id }}">
                    <button type="button" onclick="removeChiffreVision(this)" 
                        class="absolute top-5 right-3 text-red-500  p-1.5 rounded-full hover:text-red-600 transition-colors">
                        <i class="fa fa-trash text-xs"></i>
                    </button>
                    <input type="hidden" id="chiffre_vision_id-{{ $index }}" name="chiffre_vision_id[]" value="{{ $chiffre->id }}">
...
<div class="bg-white p-8 mb-8 rounded-3xl shadow-sm border border-slate-100">
    <div class="flex items-center justify-between">
        <h3 class="text-lg font-bold text-sonec-dark mb-6 flex items-center gap-2"><i class="fas fa-cogs text-green-500"></i> Section "Gouvernance"</h3>
        <button onclick="addGouvernance() " class="flex items-center gap-2 bg-sonec-green text-white px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">
            <i class="fas fa-plus"></i> Ajouter une gouvernance
        </button>
    </div>
    <div id="gouvernances-container" class="space-y-6">
        <div class="grid md:grid-cols-2 gap-6" id="gouvernances-grid">
            @foreach ($section_gouvernances as $index => $gouvernance)
                <div data-gouvernance class="bg-white border border-slate-200 rounded-xl p-4 relative" data-id="{{ $gouvernance->id }}">
                    <button type="button" onclick="removeGouvernance(this)" 
                        class="absolute top-5 right-3 text-red-500  p-1.5 rounded-full hover:text-red-600 transition-colors">
                        <i class="fa fa-trash text-xs"></i>
                    </button>
                    <input type="hidden" id="gouvernance_id-{{ $index }}" name="id[]" value="{{ $gouvernance->id }}">
                    <input type="hidden" id="gouvernance_section_key-{{ $index }}" name="gouvernance_section_key[]" value="gouvernance">
                    <label for="gouvernance_icon-{{ $index }}" class="block text-xs font-bold text-slate-400 uppercase mb-1">Icône (classe FontAwesome)</label>
                    <input type="text" name="gouvernance_icon[]" id="gouvernance_icon-{{ $index }}"
                        class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-mono text-blue-500" 
                        value="{{ $gouvernance->icon }}"
                        placeholder="fas fa-cog"> 
                    <label for="gouvernance_title-{{ $index }}" class="block text-xs font-bold text-slate-400 uppercase mb-1 mt-4">Titre</label>
                    <input type="text" name="gouvernance_title[]" id="gouvernance_title-{{ $index }}"
                        class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sonec-dark focus:ring-2 focus:ring-green-500 outline-none" 
                        value="{{ $gouvernance->title }}">
                    <label for="gouvernance_description-{{ $index }}" class="block text-xs font-bold text-slate-400 uppercase mb-1 mt-4">Description</label>
                    <textarea name="gouvernance_description[]" id="gouvernance_description-{{ $index }}"
                        class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-green-500 outline-none" 
                        rows="4">{{ $gouvernance->description }}</textarea>
                </div>
            @endforeach            
        </div>
    </div>
